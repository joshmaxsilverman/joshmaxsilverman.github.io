...
In the second case we restart the process with a roll under our belt.

So, if $T$ is the number of rolls after the first, we get

$$ T = p + (1 - p)(1 + T) = 1 + (1 - p) T $$

which leads to $T = 1/p$ and, restoring the first roll, the expected number of rolls to observe a repeat is $T^\prime = 1 + 1/p  = 7.$
...
which leads to $T = (1 + p)/p^2 = 6$ and the expected number of rolls to observe three in a row is $T^\prime = 6 + 1 = 7.$
...
$$ \left(\begin{array}{c}P_t(1) \\ P_t(2) \\ P_t(3)\end{array}\right) = \overbrace{\left(\begin{array}{ccc} (1-p) & (1-p) & 0 \\ p & 0 & 0 \\ 0 & p& 0\end{array}\right)}^\mathbf{M} \cdot \left(\begin{array}{c} P_{t-1}(1) \\ P__{t-1}(2) \\ P__{t-1}(3)\end{array}\right) $$
...
$$ \mathbf{P}_{t} = \mathbf{T}\mathbf{D}^{t-1}\mathbf{T}^{-1}\cdot\mathbf{P}_{1} $$

Setting $\mathbf{P}_1$ to $\left(1,0,0\right)^\T$ and dotting the result with $\left(0,0,1\right),$ we can extract $P_t(3).$

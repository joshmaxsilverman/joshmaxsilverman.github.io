...
$$ P_t(1) = (1-p)P_{t-1}(1) + (1-p)P_{t-1}(2). $$
...
$$ P_t(1) = \frac12 P_{t-1}(1) + \frac{1}{2^2} P_{t-2}(1). $$

If we define $\widetilde{P}_t = 2^{t-1} P_t,$ this equation becomes

$$ \widetilde{P}_t(1) = \widetilde{P}_{t-1}(1) + \widetilde{P}_{t-2}(1) $$

which is just the ordinary Fibonacci sequence, so $P_t(1) = F_t/2^{t-1}.$

Now, using the second and third equations again, we get 

$$ 
\begin{align}
   P_t(1) &= \frac12 P_{t-1}(1) \\
   &= \frac{1}{2^2} P_{t-2}(1) \\
   &= \frac{F_{t-2}}{2^{t-1}}.
\end{align}
$$
...
We can also show this through diagonalization. Packaging the system of equations, we get the linear transformation:

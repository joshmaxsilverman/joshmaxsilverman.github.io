...
With a flat metric, this ought to be zero since partial derivatives commute, but in curved space the effect of moving in two directions in either order is not the same! So, we anticipate this to come about from the coordinate changing parts of the covariant derivative.
...
We can evaluate $D_bD_aV_r$ and examine its symmetries to see which terms will survive swapping $a\leftrightarrow b.$
...
In anticipation of the subtraction of $D_aD_bV_r$ we can ignore the terms that are symmetric in $a,b$ so
...
Swapping the sum labels $\ell$ and $k$ in the double Christoffel terms, the curvature tensor $\mathcal{R}^k_{bar}$ becomes
...
Likewise, $\Gamma^\theta_{\phi\theta} = \Gamma^\theta_{\theta\phi} = 0$ as are $\Gamma^\phi_{\phi\phi}$ and $\Gamma^\phi_{\theta\theta}.$ The nonzero symbols are
...
On the parallel, $\theta=\theta_0$ so we can forget about $D_\theta t^\phi$ and $D_\theta t^\theta.$ This leaves us with the non-trivial equations $D_\phi t^\theta = 0$ and $D_\phi t^\phi = 0$ giving
...
Because the derivatives $\dot{X}^j$ are independent of the $X^j,$ the only term on the left side is 

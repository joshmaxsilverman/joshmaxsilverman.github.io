...
Update: Examining that last equation, we can see that it is the expected number of button-presses once one lands in the press-Next group ($k/2$) plus the expected number of presses of Random to land one in the press-Next group ($100/(k+1)$), minus the one virtual press of Random that has landed you on a random track at the start. Which is probably a more revealing solution!

...
for example, sheep A's boundary of vision can range from being parallel to the $AB$ line to being parallel with the $AC$ line. this is equal to $\pi - \theta$ or $\pi - \arccos\frac{\left(r_B - r_A\right)\cdot\left(r_C - r_A\right)}{\lvert r_B - r_A\rvert\lvert r_C - r_A\rvert}.$
...
$$ \frac{1}{\left(2\pi\right)^3}\int*0^1\text{d}x_1 \int_0^1\text{d}y_1\int_0^1\text{d}x_2\int_0^1\text{d}y_2 \int_0^1\text{d}x_3 \int_0^1\text{d}y_3 \left(\pi-\theta*{BAC}\right)\left(\pi-\theta*{ACB}\right)\left(\pi-\theta*{CBA}\right). $$
...
$$ P \approx \frac{\int_0^1\text{d}\theta \int_0^{\pi-\theta}\text{d}\beta \left(\pi-\theta\right)\left(\pi-\beta\right)\left(\theta+\beta\right)}{\int_0^1\text{d}\theta \int_0^{\pi-\theta}\text{d}\beta}. $$

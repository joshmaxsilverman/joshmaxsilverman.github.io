...
The path from $(x-1)$ to $(x+1)$ ($\color{olive}{\text{yellow}}$ path) is $N-1$ steps long and the "cliff" for this segment is located one step away at $x.$ This trajectory therefore has probability $P_{(N-1)}(1).$ Since this has the sauce moving to the left, $\gamma_\text{f} = \ell$ and $\gamma_\text{b} = r.$

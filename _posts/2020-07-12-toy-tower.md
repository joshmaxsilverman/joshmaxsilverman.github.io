...
With $S(3,4),$ two of the four remaining rings can be chosen to sit at height $3$. We'll then have an $S(2,3)$ problem as well as the option to call it quits and place no further rings. However, since we have two choices of which ring to place at height $3$, this contribute $2\left(S(2,3) + 1)\right)$. We could also place ring $2$ first which would give us the choice between a $S(1,3)$ problem, or calling it quits.
...
These cases contain the general logic: if we skip ahead, so that we have extra rings, then we have $n-m+1$ equivalent choices if we want to place a ring at the next available height. Otherwise we have a unique choice at the other remaining heights. In all cases, we have a new subproblem with $m-1$ 

$$\begin{align}
S(m,n) &= (n-m+1)S(m-1,n-1) + \sum\limits_{i=1}^{m-1}\left(1 + a(i,n-1)) + 1\\
       &= n + (n-m+1) + \sum\limits_{i=1}^{m-1}a(i,n-1)
\end{align}$$

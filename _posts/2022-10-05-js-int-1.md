...
when $\ell\cos\theta > 1,$ the stick will always have a horizontal crossing and the question becomes whether it will have a second. the only way the stick can make two horizontal crossings is if $P$ starts more than $\left(2 - \ell\cos\theta\right)$ away from the edge of the unit cell. 

...
Likewise, the expectation $\langle L\rvert\text{monster not seen before day }N\rangle$ is $N$ plus $\langle L\rangle_\text{no cutoff},$ since our fortunes after day $N$ are not effected by our efforts before day $N.$

But when there's a cutoff, $\langle L\rvert\text{monster not seen before day }N\rangle$ is equal to $N,$ the number of letters sent by the time G gives up on day $N.$ 

Whether or not there's a cutoff, the value of $\langle L\rvert\text{monster seen before day }N\rangle$ is the same. Using the no cutoff case to solve for it, we get

$$
  \langle L\rvert\text{monster seen before day }N\rangle = N - \frac1p
$$

...
Both of these are tails of the Gaussian distribution which is estimated by $N(t)/\sqrt{2\pi}t.$ 

Setting both equal to $1/N$ and solving for $f_1$ and $(f_1+s_1)$ we get $f_1 \approx -\sqrt{2\log N}$ and $f_1+s_1\approx -2sqrt{\log N}.$ Solving for $s_1$ we get $s_1\approx -(2-\sqrt{2})\sqrt{\log N}.$

...
The probability that $f_j$ is less than $f_1$ is $\int\limits-{-\infty}^{f_1} \text{d}f_j\, N(f_j,1) $ and the probability that $f_j+s_j\leq f_1+s_1$ is $\int\limits-{-\infty}^{f_1+s_1} \text{d}(f_j+s_j)\, N(f_j+s_j, 2) $

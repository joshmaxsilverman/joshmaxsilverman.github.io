...
My LaTeX not being up to the challenge, I've ignored in this formula that we don't square the first term when $i=t$; I made the correct adjustment in performing the calculation. Simplifying and eliminating that error:

$$\frac{1}{20}\left(\frac{t^t}{(t+1)^t} + \sum_{i=t+1}^{19}\frac{i^t}{(i+1)^{t+1}}$$
